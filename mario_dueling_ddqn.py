...
from models import MarioDuelingNet
...
class Mario:
    def __init__(self, state_dim, action_dim, save_dir):
        ...
        self.net = MarioDuelingNet(self.state_dim, self.action_dim).float()
...
def create_video(imgs_dir, episode):
    ...
    out = cv2.VideoWriter('vis/dueling_ddqn/videos/ep_' + '{0:05d}'.format(episode) + '.mp4',cv2.VideoWriter_fourcc(*'mp4v'), 15, size)
...
def main():
    ...
    episodes = 40000
    os.makedirs('vis/dueling_ddqn/videos')
    for e in range(episodes):
        imgs_dir = "vis/dueling_ddqn/ep_" + '{0:05d}'.format(e)
